...
import collector.services.schedules as schedule_services
from collector.models import CollectionEventModel, CollectionGroupModel, ScheduleModel
from collector.schemas import ScheduleSchema
...
class TestLoadSchedules:
    ...
    async def test_success(self, mocker, async_cm):
        ...
        sqlalchemy_to_pydantic = mocker.patch(
            "collector.services.schedules.sqlalchemy_to_pydantic"
        )
        mocker.patch(
            "collector.services.schedules.get_db", return_value=async_cm(mock_session)
        )
...
class TestRetrieveEvents:
    ...
    async def test_found(self, mocker):
        ...
        mocker.patch("collector.services.schedules.CollectionEventSchema")
        mocker.patch("collector.services.schedules.get_db")
        mock_get_collection = mocker.patch(
            "collector.services.schedules.get_collection",
    ...
    async def test_not_found(self, mocker, mock_sch):
        ...
        mocker.patch("collector.services.schedules.get_db")
        mock_get_collection = mocker.patch(
            "collector.services.schedules.get_collection", return_value=None
        )
    ...
    async def test_exception(self, mocker):
        ...
        mocker.patch("collector.services.schedules.get_db")
        mock_get_collection = mocker.patch(
            "collector.services.schedules.get_collection", side_effect=Exception("error")
        )
        mock_log = mocker.patch.object(schedule_services.logger, "error")
