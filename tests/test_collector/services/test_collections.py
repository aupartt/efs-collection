...
import collector.services.collections as collection_services
from collector.schemas.collection import (
    CollectionGroupSchema,
)


class TestLoadCollectionGroups:
    @pytest.mark.asyncio
    async def test_success(self, mocker, async_cm, mock_grp_col):
        ...
        mocker.patch(
            "collector.services.collections.get_db", return_value=async_cm(mock_session)
        )
        mocker.patch(
            "collector.services.collections.sqlalchemy_to_pydantic",
...
class TestGetCollection:
    @pytest.mark.asyncio
    async def test__found(self, mock_grp_col):
        mock_session = AsyncMock(spec=AsyncSession)
        mock_results = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_not_found(self, mock_grp_col):
        mock_session = AsyncMock(spec=AsyncSession)
        mock_results = MagicMock()
...
class TestHandleLocation:
    @pytest.mark.asyncio
    async def test_found(self, mocker, async_cm, mock_loc, mock_grp_col):
        mock_session = AsyncMock(spec=AsyncSession)
        ...
        mocker.patch(
            "collector.services.collections.get_location", return_value=location_db
        )
    ...
    @pytest.mark.asyncio
    async def test_not_found(self, mocker, async_cm, mock_loc_col):
        mock_session = AsyncMock(spec=AsyncSession)

        mocker.patch(
            "collector.services.collections.get_db", return_value=async_cm(mock_session)
        )
        mocker.patch("collector.services.collections.get_location", return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_exception(self, mocker, async_cm, mock_loc_col):
        mock_session = AsyncMock(spec=AsyncSession)
        ...
        mocker.patch(
            "collector.services.collections.get_location", side_effect=Exception("error")
        )
        mock_log = mocker.patch.object(collection_services.logger, "error")
...
class TestHandleCollection:
    @pytest.mark.asyncio
    async def test_new(self, mocker, async_cm, mock_grp_col):
        mock_session = AsyncMock(spec=AsyncSession)
        ...
        mocker.patch("collector.services.collections.get_collection", return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_existing(self, mocker, async_cm, mock_grp_col):
        mock_session = AsyncMock(spec=AsyncSession)

        collection_input = mock_grp_col.schemas[0]
        collection_input.efs_id = "foo"
        collection_check = collection_input.model_copy()
        ...
        mocker.patch(
            "collector.services.collections.get_collection", return_value=collection_db
        )
    ...
    @pytest.mark.asyncio
    async def test_no_efs_id(self, mocker, async_cm, mock_grp_col):
        mocker.patch(
            "collector.services.collections.get_db", return_value=async_cm(None)
        )
        mock_log = mocker.patch.object(collection_services.logger, "warning")
    ...
    async def test_exception(self, mocker, async_cm, mock_grp_col):
        ...
        mocker.patch(
            "collector.services.collections.get_collection",
...
class TestHandleEvent:
    @pytest.mark.asyncio
    async def test_new(self, mocker, async_cm, mock_evt_col):
        mock_session = AsyncMock(spec=AsyncSession)

        mocker.patch(
            "collector.services.collections.get_db", return_value=async_cm(mock_session)
        )
        mocker.patch("collector.services.collections.get_event", return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_existing(self, mocker, async_cm, mock_evt_col):
        mock_session = AsyncMock(spec=AsyncSession)
        ...
        mocker.patch(
            "collector.services.collections.get_event",
            return_value=mock_evt_col.models[0],
        )
    ...
    @pytest.mark.asyncio
    async def test_exception(self, mocker, async_cm, mock_evt_col):
        mock_session = AsyncMock(spec=AsyncSession)
...
class TestHandleSnapshot:
    @pytest.mark.asyncio
    async def test_success(self, mocker, async_cm, mock_snap_col):
        mock_session = AsyncMock(spec=AsyncSession)
...
class TestSaveLocationCollections:
    @pytest.mark.asyncio
    async def test_success(
        self, mocker, mock_loc_col, mock_grp_col, mock_evt_col, mock_snap_col
    ):
        ...
        mocker.patch(
            "collector.services.collections._handle_location",
            side_effect=mock_handle_location,
        )
        mocker.patch(
            "collector.services.collections._handle_collection",
            side_effect=mock_handle_collection,
        )
        mocker.patch(
            "collector.services.collections._handle_event", side_effect=mock_handle_event
        )
        mocker.patch(
            "collector.services.collections._handle_snapshot",
    ...
    @pytest.mark.asyncio
    async def test_with_none_results(
        self,
        mocker,
        mock_loc_col,
    ):
        mocker.patch(
            "collector.services.collections._handle_location", return_value=None
        )
