...
import collector.services.locations as location_services
...
async def test_load_locations(mocker, async_cm, mock_loc):
    ...
    mocker.patch(
        "collector.services.locations.get_db", return_value=async_cm(mock_session)
    )
    mocker.patch(
        "collector.services.locations.sqlalchemy_to_pydantic",
...
async def test_add_location_new(mocker, async_cm, mock_loc):
    ...
    mocker.patch("collector.services.locations.get_group", return_value=mock_group)
    mocker.patch(
        "collector.services.locations.get_db", return_value=async_cm(mock_session)
    )
...
async def test_add_location_group_not_exists(mocker, async_cm, mock_loc):
    ...
    mocker.patch("collector.services.locations.get_group", return_value=None)
    mocker.patch(
        "collector.services.locations.get_db", return_value=async_cm(mock_session)
    )
...
async def test_save_locations(mocker, mock_loc):
    ...
    mocker.patch(
        "collector.services.locations.add_location", side_effect=mock_add_location
    )
