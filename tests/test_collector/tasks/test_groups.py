...
import collector.tasks.groups as tasks_groups
...
async def test_retrieve_region_found(mocker, mock_regions):
    mock_api_get_regions = mocker.patch(
        "collector.tasks.groups.api_get_regions.asyncio", return_value=mock_regions
    )
...
async def test_retrieve_groups(mocker, mock_regions, mock_grp):
    mock_api_get_groupements = mocker.patch(
        "collector.tasks.groups.api_get_groupements.asyncio",
        return_value=mock_grp.api,
    )
    mock_api_to_pydantic = mocker.patch(
        "collector.tasks.groups.api_to_pydantic", return_value=mock_grp.schemas
    )
...
async def test_update_groups_success(mocker, mock_regions, mock_grp):
    ...
    mock_check_api = mocker.patch("collector.tasks.groups.check_api", return_value=True)
    mock_retrieve_region = mocker.patch(
        "collector.tasks.groups._retrieve_region", return_value=mock_region
    )
    mock_retrieve_groups = mocker.patch(
        "collector.tasks.groups._retrieve_groups", return_value=mock_grp.schemas
    )
    mock_save_groups = mocker.patch("collector.tasks.groups.save_groups")
    mock_settings = mocker.patch("collector.tasks.groups.settings")
...
async def test_update_groups_api_check_fails(mocker):
    mock_check_api = mocker.patch("collector.tasks.groups.check_api", return_value=False)
    mock_retrieve_region = mocker.patch("collector.tasks.groups._retrieve_region")
    mock_retrieve_groups = mocker.patch("collector.tasks.groups._retrieve_groups")
    mock_save_groups = mocker.patch("collector.tasks.groups.save_groups")
...
async def test_update_groups_empty_groups(mocker, mock_regions):
    ...
    mock_retrieve_groups = mocker.patch(
        "collector.tasks.groups._retrieve_groups", return_value=empty_groups
    )
    mock_save_groups = mocker.patch("collector.tasks.groups.save_groups")
    mock_settings = mocker.patch("collector.tasks.groups.settings")
    mock_settings.REGION_NAME = mock_region.libelle
